...
fn eval_gte(val_left: &Value, val_right: &Value, _document: &CoreDocument) -> Value {
...
pub fn eval<'a>(
    ...
    spill_eval::eval_two_arity_spill(context, document, args, eval_ref_handle, eval_gte)
}
...
mod tests {
    ...
    fn test_gte_false() {
        let (context, doc) = test_utils::setup(A2.clone(), |doc| {
            doc.insert_cell_facts(
                &test_utils::SHEET1_A1_COORD,
                vec![vec![Some("=1".to_string()), Some("=A1 >= 1.1".to_string())]],
            )
            .unwrap();
